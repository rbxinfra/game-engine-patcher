...
DYNAMIC_LOGVARIABLE(Proxy, 0);

namespace rbx { namespace proxy {
	void load_version_library()
	{
		FASTLOGFMT(DFLog::Proxy, "Loading version library");

		wchar_t szSysWow64Path[MAX_PATH];
		{
			GetSystemWow64DirectoryW(szSysWow64Path, MAX_PATH);
		}

		unload_version_library();

		wchar_t szDllPath[MAX_PATH];
		::swprintf_s(szDllPath, MAX_PATH, L"%s\\%s.dll", szSysWow64Path, szDllName);
	...
	{
		FASTLOGFMT(DFLog::Proxy, "Unloading version library");
